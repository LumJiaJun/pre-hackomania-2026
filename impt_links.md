API Get Weather Call  
https://api.open-meteo.com/v1/forecast?latitude=1.3521&longitude=103.8198&current=temperature_2m,weathercode,windspeed_10m&timezone=Asia%2FSingapore  

Open-payments  
 Github: https://github.com/interledger/open-payments  
 Docs: https://openpayments.dev/overview/getting-started/  
 SDK: https://openpayments.dev/sdk/before-you-begin/

Google ADK:  
 Docs: https://google.github.io/adk-docs/  
 Python ADK Github: https://github.com/google/adk-python  
 Python ADK Docs: https://google.github.io/adk-docs/api-reference/python/  
 REST API Docs: https://google.github.io/adk-docs/api-reference/rest/  
 CLI Reference: https://google.github.io/adk-docs/api-reference/cli/  
 AgentConfig Reference: https://google.github.io/adk-docs/api-reference/agentconfig/  

ClickHouse:  
 SQL LINQ e.g. SELECT name, score FROM users WHERE score > 100 ORDER BY score DESC;  
 Demo App: https://github.com/ClickHouse/HouseClick  
 Docs: https://clickhouse.com/docs/use-cases/AI/MCP/librechat  

OPENED TABS  
https://github.com/lohjo/pre-hackomania-2026  
https://claude.ai/chat/d7410576-ee6f-4e98-b888-6774434a0b70  
https://openpayments.dev/overview/getting-started/  
https://google.github.io/adk-docs/get-started/python/  
https://google.github.io/adk-docs/api-reference/rest/#run  
https://google.github.io/adk-docs/api-reference/cli/  
https://google.github.io/adk-docs/api-reference/agentconfig/  
https://clickhouse.com/docs/use-cases/AI/MCP/librechat  
https://www.docker.com/products/docker-desktop/  
https://www.google.com/search?client=opera-gx&q=how+do+I+know+mine+is+AMD+or+ARM+64&sourceid=opera&ie=UTF-8&oe=UTF-8  

Interledger:
Slides: https://docs.google.com/presentation/d/13DZjPFNC4HZr0wnYDC3BOI9VvcvY0KK-1VwbTYfpIi4/edit
